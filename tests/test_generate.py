...
class TestGenerate(unittest.TestCase):
    ...
    def test_stream_generate_speculative_cancel_during_prefill(self):
        draft_model = self.model
        sampler = make_sampler(temp=0.0)
        prompt = self._chat_prompt_tokens("Please cancel during speculative prefill. " * 16)
    ...
    def test_stream_generate_speculative_cancel_before_first_step(self):
        draft_model = self.model
        sampler = make_sampler(temp=0.0)
        prompt = self._chat_prompt_tokens("Please cancel before speculative first-step. " * 16)
