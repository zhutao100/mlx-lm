...
class RotatingKVCache(_BaseCache):
    ...
    def to_quantized(self, group_size: int = 64, bits: int = 4) -> QuantizedKVCache:
        quant_cache = QuantizedRotatingKVCache(max_size=self.max_size, keep=self.keep, group_size=group_size, bits=bits)
        quant_cache.offset = self.offset
...
class QuantizedRotatingKVCache(QuantizedKVCache):
    def __init__(self, max_size: int, keep: int = 0, group_size: int = 64, bits: int = 8):
    ...
    def _update_in_place(self, keys, values):
        ...
        if self.keys is None or (prev >= self.keys[0].shape[-2] and self.keys[0].shape[-2] < self.max_size):
            new_size = min(self.step, self.max_size - prev)
            new_k = init_quant(new_size, k_head_dim, keys.dtype)
            new_v = init_quant(new_size, v_head_dim, values.dtype)

            if self.keys is not None:
                self.keys = tuple(mx.concatenate([x, nx], axis=-2) for x, nx in zip(self.keys, new_k))
                self.values = tuple(mx.concatenate([x, nx], axis=-2) for x, nx in zip(self.values, new_v))
            else:
                self.keys = new_k
                self.values = new_v
            self._idx = prev
